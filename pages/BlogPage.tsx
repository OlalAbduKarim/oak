                ...
                <div key={post.id} className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col group">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <p className="mt-4 text-gray-600 flex-grow">
                      {isExpanded ? post.fullText : post.excerpt}
                    </p>
                    <button
                      onClick={() => toggleReadMore(post.id)}
                      className="mt-6 inline-flex items-center text-accent-gold font-semibold self-start"
                      aria-expanded={isExpanded}
                    >
                      {isExpanded ? 'Read less' : 'Read more'}
                      <ArrowRightIcon className={`ml-2 w-5 h-5 transition-transform duration-300 ${isExpanded ? '-rotate-90' : 'group-hover:translate-x-1'}`} />
                    </button>
                  </div>
                </div>
              );
